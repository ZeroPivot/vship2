...
class Controller
	def initialize(screen)
		...
		@visualize = Visualize.new(WIDTH, HEIGHT)
	...
	def run
		loop do
			 ...
			 case event
				...
				when Rubygame::KeyDownEvent				
						...
						when Rubygame::K_RETURN
							a = Projectile.new(
							UnitVector.get(@player.vector,@mouse.vector),
							ThetaQuadrant.get(@player.vector,@mouse.vector),
							@player.centerx,
							@player.centery )
							#puts a
							a.speed=1 #lol
							...
							@projectile_grp << a # add projectile to its group
							@visualize << a # add this bullet to the visualize set, creates a velocity vector of any given bullet in the @bullets group,
							                # pointing out from the bullet
			...
			if @player.dirBits? > 0
				...
				elsif @player.centerx > WIDTH
					@player.centerx = 0
				end	
				if @player.centery > HEIGHT
					@player.centery = 0
			...
			#print "snap"
			ThetaQuadrant.get(@player.vector,@mouse.vector)
			@player.rotate(ThetaQuadrant.get(@player.vector,@mouse.vector))#updateMousePos() ####
			@player.draw(@screen)
